...
call LunarSaveRAMFileEx().  Or you can specify the LC_SAVEONCLOSE option
...
void* data            - Pointer to array of bytes to use as file (or if the 
                        LC_CREATEARRAY option is used, this is the null
                        terminated string for the name of file to open).
unsigned int FileMode - Indicates the mode to open the file in, plus optional
                        flags.  The modes and flags available are:
 LC_READONLY        : Open existing file in read-only mode (default).
 LC_READWRITE       : Open existing file in read and write mode.
 LC_CREATEREADWRITE : Create a new file in read and write mode, erase the
                      existing file (if any).
 LC_UTF8            : When the data argument is a file name string, this
                      indicates that the string is in UTF-8 format.  If
                      this flag is not specified, it is assumed to use
                      the current "ANSI" code page of the OS instead.
...
char* FileName        - Null terminated path and name of the file to save as.
                        You can set this to NULL if you used the
                        LC_CREATEARRAY flag in LunarOpenRAMFile and you wish
                        to save to the same file you opened, but only if it
                        was not opened in read-only mode.
unsigned int Flags    - Indicates optional flags.  The flags available are:
 LC_UTF8            : The FileName argument will be in UTF-8 format.  If
                      this flag is not specified, it is assumed to use
                      the current "ANSI" code page of the OS instead.
...
typedef unsigned int (WINAPI *LCPROC27)(char* FileName, unsigned int Flags);
extern LCPROC27 LunarSaveRAMFileEx;
//extern "C" unsigned int WINAPI _export LunarSaveRAMFileEx(char* FileName, unsigned int Flags)



/**************************************************************************

Depreciated in 1.90.  This just calls LunarSaveRAMFileEx() with the Flags
value set to 0.
...
unsigned int ROMType - One of the following values:
    ...
    LC_SUPERFXROM   : For Super FX games up to 64 Mbit ROM sizes (see note).
    LC_SUPERFXROM_2 : Same effect as LC_SUPERFXROM.
...
Note that the SuperFX chip itself cannot access past 2 MB meaning only
the SNES CPU can access the rest, so no commercial games bothered with
that setup and currently most emulators don't support beyond 2 MB for
SuperFX ROMs anyway.
...
unsigned int ROMType - One of the following values:
    ...
    LC_EXHIROM      : For HiROM games above 32 Mbit.
    LC_SUPERFXROM   : For Super FX games up to 64 Mbit ROM sizes (see note).
    LC_SUPERFXROM_2 : Uses 40:0000 map when possible.
unsigned int Header  - LC_NOHEADER or LC_HEADER (0x200 bytes).
...
LC_SUPERFXROM_2 uses the 40:0000 map for the first 2 MB of the ROM.
...
___________________________________________________________________________
 ...
 LC_LZ19   Kirby Super     A somewhat less advanced version of the LC_LZ3
           ...
           Adventure [G]
           Earthbound [G]
 LC_LZ20   Marvelous [G]   An extended form of LZ2.
...
unsigned int Mbits - MegaBits to expand the ROM to (0-32).
   ...
   LC_64_EXLOROM_3 : Higher compatibility, but uses up most of the new space.
                     Do not use this unless the ROM doesn't load or has
                     problems with the other options.
   LC_48_EXPCHIP   : For expansion chips SA-1, SDD-1, and SuperFX
   LC_64_EXPCHIP   : For expansion chips SA-1, SDD-1, and SuperFX
...
than 48 Mbits (6 MB).  However you can download a custom unofficial
1.51 ZSNES 8MB build from FuSoYa's site.

Options are provided for SA-1 and SDD-1, however these are MMC chips
that can do bank switching to change which parts of the ROM are
accessed for a given bank.  You may need to change the MMC registers
with ASM to access areas past the 4 MB mark.  If you have a game-
specific tool (like Lunar Magic for SMW with the SA-1 patch), it's
better to use that for expansion instead.  You can also use these
same options for SuperFX games, however the SuperFX chip itself
cannot access past 2 MB meaning only the SNES CPU can access the rest,
so no commercial games bothered with that setup and currently most
emulators don't support beyond 2 MB for SuperFX ROMs anyway.
...
char* IPSFileName   - Null terminated file name and path of the IPS file
                      to create (optional).  If you set this to NULL, the
                      function will prompt the user for a file name.
char* ROMFileName   - Null terminated file name and path of UNMODIFIED ROM
                      to use (optional).  If you set this to NULL, the
                      function will prompt the user for a file name.
char* ROM2FileName  - Null terminated file name and path of MODIFIED ROM
                      to use (optional).  If you set this to NULL, the
                      function will prompt the user for a file name.
unsigned int Flags  - optional flags.  This can be any combination of:
 LC_IPSQUIET            : Suppress all message boxes other than file name
                          prompts.
 LC_IPSFORCEFILE_SAVEAS : Force bringing up the file "save as" dialog
                          even if a file name was provided.
 LC_IPS_ALLOW_MENU      : Allow extra menu items on the right click context 
                          menus of the window title bar for ROM file pickers
                          (for add/remove SNES header, etc).
 LC_IPS_ADD_HEADER      : Adds 0x200 byte SNES copier headers to both the
                          original and modified files before creating the
                          patch, but only if they don't already have one.
 LC_IPS_REMOVE_HEADER   : Similar to LC_IPS_ADD_HEADER except it removes
                          the SNES copier header from the files, but only
                          if they already have one.  Do not combine with
                          LC_IPS_ADD_HEADER.
 LC_UTF8                : The file name arguments will be in UTF-8 format.
                          If this flag is not specified, they are assumed
                          to use the current "ANSI" code page of the OS
                          instead.
...
char* IPSFileName   - Null terminated file name and path of the IPS file
                      to use (optional).  If you set this to NULL, the
                      function will prompt the user for a file name.
char* ROMFileName   - Null terminated file name and path of ROM to apply
                      patch to (optional).  If you set this to NULL, the
                      function will prompt the user for a file name.
unsigned int Flags  - optional flags.  This can be any combination of:
 ...
 LC_IPS_ALLOW_MENU      : Allow extra menu items on the right click context 
                          menus of the window title bar for ROM file pickers
                          (for add/remove SNES header, etc).
 LC_IPS_ADD_HEADER      : Adds 0x200 byte SNES copier header to the file to
                          be patched before patching, but only if it doesn't
                          already have one.
 LC_IPS_REMOVE_HEADER   : Similar to LC_IPS_ADD_HEADER except it removes
                          the SNES copier header from the ROM, but only
                          if it already has one.  Do not combine with
                          LC_IPS_ADD_HEADER.
 LC_UTF8                : The file name arguments will be in UTF-8 format.
                          If this flag is not specified, they are assumed
                          to use the current "ANSI" code page of the OS
...
Lunar Magic 1.40+ as an improved method of verifying free space and storing
